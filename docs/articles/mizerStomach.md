...
predator’s prey are distributed over body size (Hyslop 1980; Baker,
Buckland, and Sheaves 2014; Amundsen, Gabler, and Staldal 1996). It
works on the log predator/prey mass ratio

$$\log\left( {PPMR} \right) = \log\left( \frac{w_{\text{pred}}}{w_{\text{prey}}} \right).$$

fits a probability distribution to $\log\left( {PPMR} \right)$, and can
translate that distribution into feeding-kernel parameters for a
[mizer](https://sizespectrum.org/mizer/) model (Scott, Blanchard, and
Andersen 2014).
...
normal mixture with weighted EM updates and can be transferred to mizer
as a `"gaussian_mixture"` predation kernel. Mixture fits can be
sensitive to initialization and nearly degenerate components, so check
the result carefully before using it in a model.
...
supports normal, truncated-exponential, and Gaussian-mixture fits. It
first transforms the fit from its recorded `power` to the exponent
...
feeding kernel, a truncated exponential to its `"power_law"` kernel, or
a Gaussian mixture to its `"gaussian_mixture"` kernel.
